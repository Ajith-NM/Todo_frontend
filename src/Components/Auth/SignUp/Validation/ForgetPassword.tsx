...
const ForgetPassword = () => {
  ...
  const { errors } = formState
   ...
   const onSubmit=(data:FormValues)=>{
    request.post("user/forgetPassword",data).then((data:AxiosResponse)=>{
  ...
  return (
    ...
    <form className="loginform validation" onSubmit={handleSubmit(onSubmit)}>
     ...
     <input type="text" className="inputfield"  placeholder="Email" {...register("email", {
            required: {
              value: true, message: "please enter email"
            },
            pattern: {
              value: /^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$/, message: "please enter the correct format email"
            },
          })} />
