...
const ResetPassword = () => {
  ...
  const { errors } = formState
   ...
   const onSubmit=(data:FormValues)=>{
    if (data.password[0]===data.password[1]) {
      request.put("user/resetPassword",{password:data.password[0]}).then((data:AxiosResponse)=>{
  ...
  return (
    ...
    <form className="loginform validation" onSubmit={handleSubmit(onSubmit)}>
     ...
     <input type="text" className="inputfield resetPassword"  placeholder="New Password" {...register("password.0", {
            required: {
          ...
          <p style={{color:"brown" ,fontSize:"16px"}}>{errors.password?.[0]?.message}</p>
     <input type="text" className="inputfield "  placeholder="Confirm Password" {...register("password.1", {
            required: {
              value: true, message: "please enter password"
            },
            minLength: {
          ...
          <p style={{color:"brown",fontSize:"16px"}}>{errors.password?.[1]?.message}</p>
     </div>
